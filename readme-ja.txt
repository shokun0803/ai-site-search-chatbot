...
* 設定不備・プロバイダーエラー・検索結果なし・利用上限到達などで AI 以外の代替応答になったチャットログに「再取得」ボタンを表示。再取得が成功した場合は Saved Knowledge Base の下書きを自動生成し、再取得しても失敗する場合は質問を無駄にしないよう回答欄が空の下書きを自動作成
...
= 1.1.0 =
* 設定不備・プロバイダーエラー・検索結果なし・利用上限到達などで AI 以外の代替応答になったチャットログに「再取得」ボタンを追加。訪問者の質問を検索と AI 回答生成にもう一度かけ直せるようにした
* 再取得が成功した場合、既存の自動下書き機能を利用して新しい回答を Saved Knowledge Base の下書きとして保存（有効化している場合）
* 再取得しても AI 回答が得られなかった場合は、質問だけを保持した回答欄が空の下書きエントリを自動作成し、2 回失敗した質問もログの中に埋もれず手動回答の対象として残るようにした
* 不具合修正: ルールベースの検索キーワード抽出処理で、`trim()` の文字マスク引数がバイト単位で除去を行うため、日本語などのマルチバイト文字を含む質問の先頭文字が破損する場合があった。この問題によりサイト内検索の一致精度が意図せず下がる、また今回の再取得機能でナレッジ下書きに保存される質問文が破損する可能性があったため修正した
